...
class IvFailureController @Inject() (
    ...
    request.userAnswers.get(IdentifierPage) match {
      case Some(identifier) =>
        ...
        queryString.fold {
          ...
          auditService.auditFailure(
            CLAIM_A_TRUST_FAILURE,
            identifier,
            FailureReasons.IV_TECHNICAL_PROBLEM_NO_JOURNEY_ID
          )
  ...
  def trustNotFound: Action[AnyContent] = actions.authWithData.async { implicit request =>
  ...
  def trustStillProcessing: Action[AnyContent] = actions.authWithData.async { implicit request =>
    request.userAnswers.get(IdentifierPage) map { identifier =>
      logger.info(
        s"[IvFailureController][trustStillProcessing][Session ID: ${Session.id(hc)}]" +
