...
class SaveIdentifierController @Inject() (
  ...
  def save(identifier: String): Action[AnyContent] = actions.authWithOptionalData.async { implicit request =>
    val result = for {
    ...
    result.value.flatMap {
      ...
      case Left(_)                 =>
        logger.warn(
          s"[$className][save][Session ID: ${Session.id(hc)}] " +
  ...
  def getIdentifier(
    identifier: String
  )(implicit request: OptionalDataRequest[AnyContent]): Either[TrustErrors, String] = identifier match {
    case IdentifierRegex.UtrRegex(utr) => Right(utr)
    case IdentifierRegex.UrnRegex(urn) => Right(urn)
    case _                             =>
      logger.error(
        s"[$className][getIdentifier][Session ID: ${Session.id(hc)}] " +
  ...
  private def checkIfAlreadyHaveIvRelationship(
    identifier: String
  )(implicit request: OptionalDataRequest[AnyContent]): TrustEnvelope[Result] = EitherT {
    relationship.check(request.internalId, identifier).value flatMap {
      case Right(RelationshipFound)    =>
        logger.info(
          s"[$className][checkIfAlreadyHaveIvRelationship][Session ID: ${Session.id(hc)}] " +
